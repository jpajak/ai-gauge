...
- Developers or technical users using Claude, Codex, OpenCode, Copilot, and OpenRouter enough to care about rolling, session, weekly, monthly, or spend limits.
...
- Sign in to Claude, ChatGPT Codex, or OpenCode through embedded Chromium; paste cookies as a fallback; and manage multiple named Claude/Codex accounts.
- Configure OpenCode with a workspace Go usage-page URL.
...
- Hypothesis: combining Claude, Codex, OpenCode, Copilot, and OpenRouter usage in one compact desktop surface may distinguish it from provider-specific dashboards.
...
- The app is unofficial and not affiliated with Anthropic, OpenAI, OpenCode, GitHub, Microsoft, OpenRouter, or other providers.
- Claude, Codex, and OpenCode scraping depends on provider web page structure; layout, authentication, Cloudflare/security checks, or upstream UI changes can break reads.
- Codex may temporarily omit its five-hour Session card; AI Gauge continues showing Weekly usage and pauses session-to-weekly calibration until the card returns.
