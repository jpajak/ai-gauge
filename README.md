...
- **Windows / Linux:** the widget floats above other windows by default. Drag anywhere to move, or drag the bottom-right corner to resize the full view; the chosen size is restored on the next launch. The down/up controls switch between full and compact views, the dash hides the window to the system tray, and the ✕ quits AI Gauge completely. Right-click the tray icon for Refresh / Settings / Quit. Left-click toggles widget visibility. The tray dot uses the configured range and color of the most severe enabled metric.
